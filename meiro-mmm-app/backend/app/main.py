...
def _save_runs():
    ...
    try:
        ...
        for rid, r in RUNS.items():
            out[rid] = {k: v for k, v in r.items() if k in ("status", "stage", "progress_pct", "config", "kpi_mode", "created_at", "updated_at", "dataset_id", "r2", "contrib", "roi", "engine", "detail", "uplift", "campaigns", "channel_summary", "adstock_params", "saturation_params", "diagnostics", "attribution_model", "attribution_config_id", "stale_from_status", "stale_reason", "stale_at")}
...
def _refresh_journey_aggregates_after_import(db, *, reprocess_days: Optional[int] = None) -> None:
    ...
    try:
        from app.services_rebuild_jobs import rebuild_journey_aggregate_outputs

        metrics = rebuild_journey_aggregate_outputs(db, reprocess_days=reprocess_days)
        logger.info(
            ...
            metrics.get("effective_reprocess_days"),
        )
    except Exception as e:
...
app.include_router(
    create_journeys_router(
        ...
        get_settings_obj=lambda: SETTINGS,
        rebuild_journey_definition_outputs_fn=lambda db, definition_id, reprocess_days=None: rebuild_journey_definition_outputs(
            db,
...
app.include_router(
    create_config_management_router(
        get_db_dependency=get_db,
        require_permission_dependency=require_permission,
        ...
        activate_config_fn=activate_config,
        ...
        list_journey_settings_versions_fn=list_journey_settings_versions,
        create_journey_settings_draft_fn=create_journey_settings_draft,
        get_journey_settings_version_fn=get_journey_settings_version,
...
app.include_router(
    create_meiro_integration_router(
        get_db_dependency=get_db,
        get_base_url_fn=lambda: BASE_URL,
        get_data_dir_obj=lambda: DATA_DIR,
        get_datasets_obj=lambda: DATASETS,
        get_settings_obj=lambda: SETTINGS,
        meiro_parser_version=MEIRO_PARSER_VERSION,
        import_journeys_from_cdp_fn=lambda *args, **kwargs: import_journeys_from_cdp(*args, **kwargs),
        from_cdp_request_factory=FromCDPRequest,
        attribution_mapping_config_cls=AttributionMappingConfig,
        canonicalize_meiro_profiles_fn=canonicalize_meiro_profiles,
...
app.include_router(
    create_data_sources_router(
        get_db_dependency=get_db,
        list_data_sources_fn=list_data_sources,
        create_data_source_fn=create_data_source,
        update_data_source_fn=update_data_source,
        test_data_source_payload_fn=test_data_source_payload,
        test_saved_data_source_fn=test_saved_data_source,
        disable_data_source_fn=disable_data_source,
        delete_data_source_fn=delete_data_source,
        rotate_data_source_credentials_fn=rotate_data_source_credentials,
        get_data_dir_obj=lambda: DATA_DIR,
        get_sample_dir_obj=lambda: SAMPLE_DIR,
        get_datasets_obj=lambda: DATASETS,
        get_expenses_obj=lambda: EXPENSES,
        get_import_sync_state_obj=lambda: IMPORT_SYNC_STATE,
        get_sync_in_progress_obj=lambda: SYNC_IN_PROGRESS,
        now_iso_fn=_now_iso,
        ...
        get_meiro_readiness_fn=lambda: build_meiro_readiness(
            ...
            meiro_config={
                ...
                "event_webhook_url": f"{BASE_URL}/api/connectors/meiro/events",
                "webhook_last_received_at": get_webhook_last_received_at(),
                "webhook_received_count": get_webhook_received_count(),
