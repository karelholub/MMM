...
from app.models_config_dq import ConversionPath, FunnelDefinition, JourneyDefinition, SilverConversionFact, SilverTouchpointFact
...
def test_funnel_diagnostics_uses_instance_facts_when_silver_and_paths_absent():
    db = _unit_db_session()
    try:
        jd = JourneyDefinition(
            id="jd-instance-diag",
        ...
        funnel = FunnelDefinition(
            id="f-instance-diag",
            journey_definition_id="jd-instance-diag",
            workspace_id="default",
            user_id="u1",
            name="Checkout funnel",
            steps_json=[
        ...
        inserted = persist_journeys_as_conversion_paths(
            ...
            replace=True,
            import_source="meiro_events_replay",
            import_batch_id="instance-diag-batch",
        )
        assert inserted == 4
